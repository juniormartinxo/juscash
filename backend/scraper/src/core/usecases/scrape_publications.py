...
from src.shared.value_objects import ScrapingCriteria, DJEUrl, Status
from src.shared.exceptions import (
    DJEScrapingException,
    ApiException,
)
...
class ScrapePublicationsUseCase:
    ...
    async def execute(
        ...
        try:
            ...
            if not caderno_success:
                raise DJEScrapingException(
                    f"Falha ao acessar {criteria.get_caderno_description()}"
                )

            logger.info(f"Acesso ao {criteria.get_caderno_description()} realizado")

            # 5. Extrair publicações
            publications = await self.scraper.extract_publications(
                criteria, max_publications
            )
            ...
            for publication in publications:
                try:
                    await self._process_single_publication(
                        publication, skip_duplicates, stats
                    )
                except Exception as e:
                    logger.error(
                        f"Erro ao processar publicação {publication.id}: {str(e)}"
                    )
            ...
            logger.info(
                f"Estatísticas: {stats['publications_new']} novas, "
            ...
            stats["errors"].append(
                {"general_error": str(e), "timestamp": datetime.now().isoformat()}
            )
            raise
    ...
    async def _process_single_publication(
        self, publication: Publication, skip_duplicates: bool, stats: dict
    ) -> None:
        """
        ...
        try:
            ...
            detailed_publication = await self.scraper.extract_publication_details(
                publication
            )
            ...
            if not detailed_publication.is_valid_for_processing():
                logger.warning(
                    f"Publicação {detailed_publication.id} não tem dados suficientes"
                )
                stats["publications_failed"] += 1
                return
            ...
            if skip_duplicates and detailed_publication.process_number:
                exists = await self.api.exists_by_process_number(
                    detailed_publication.process_number
                )
                if exists:
                    logger.info(
                        f"Publicação duplicada ignorada: {detailed_publication.process_number}"
                    )
                    stats["publications_duplicated"] += 1
                    return

            # 4. Garantir que réu seja sempre INSS (regra de negócio)
            detailed_publication.defendant = (
                "Instituto Nacional do Seguro Social - INSS"
            )
            detailed_publication.status = Status.NEW

            # 5. Salvar no banco de dados
            saved_publication = await self.api.save_publication(detailed_publication)
            ...
            logger.error(f"Erro de API ao processar publicação: {str(e)}")
            raise
        except Exception as e:
