"""
Parser aprimorado para extrair publicações do DJE-SP seguindo instruções específicas
Lida com publicações divididas entre múltiplas páginas PDF
"""
...
class EnhancedDJEContentParser:
    ...
    4. Lida com publicações divididas entre páginas
    """
    ...
    def set_scraper_adapter(self, scraper_adapter):
        """Define o adapter do scraper para baixar páginas anteriores quando necessário"""
    ...
    async def _extract_publication_for_rpv_occurrence(
        ...
        process_match = self._find_process_before_position(content, rpv_position)
        ...
        if not process_match:
            ...
            if current_page_number and current_page_number > 1:
                ...
                previous_page_content = await self._download_previous_page(source_url, current_page_number - 1)
                if previous_page_content:
                    # Buscar o último processo na página anterior
                    combined_content = previous_page_content + content
                    process_match = self._find_process_before_position(combined_content, len(previous_page_content) + rpv_position)
                    
                    if process_match:
                        logger.info("✅ Processo encontrado na página anterior")
                        content = combined_content  # Usar conteúdo combinado
                        rpv_position = len(previous_page_content) + rpv_position  # Ajustar posição
                    else:
                        logger.warning("⚠️ Processo ainda não encontrado mesmo com página anterior")
                        return None
                else:
        ...
        publication_end = self._find_publication_end(content, process_start)
        
        # 3. Extrair conteúdo completo da publicação
        publication_content = content[process_start:publication_end]
    ...
    async def _download_previous_page(self, source_url: str, page_number: int) -> Optional[str]:
        """
        Baixa e extrai conteúdo da página anterior para casos de publicações divididas
        """
        if not self.scraper_adapter:
            logger.warning("⚠️ Scraper adapter não configurado, não é possível baixar página anterior")
            return None
        
        try:
            # Construir URL da página anterior
            # Assumindo que source_url contém nuSeqpagina
            previous_url = re.sub(r'nuSeqpagina=\d+', f'nuSeqpagina={page_number}', source_url)
            
            logger.info(f"📥 Baixando página anterior: {previous_url}")
            
            # Usar o scraper adapter para baixar a página anterior
            # Implementação simplificada - pode ser expandida conforme necessário
            
            return None  # Por ora, retornar None - implementação completa requer integração com scraper
            
        except Exception as e:
            logger.error(f"❌ Erro ao baixar página anterior: {e}")
            return None
