...
class DatabaseConnectionManager:
    ...
    def verify_database_schema(self) -> bool:
        ...
        try:
            with db_config.get_session() as session:
                ...
                essential_columns = [
                    'id', 'process_number', 'availability_date', 
                    'authors', 'content', 'status', 'created_at'
                ]
                ...
                for column in essential_columns:
                    ...
                    if not result.scalar():
                        logger.error(f"Coluna '{column}' não encontrada na tabela 'publications'")
                        return False
    ...
    def get_database_info(self) -> Optional[dict]:
        ...
        try:
            with db_config.get_session() as session:
                ...
                count_result = session.execute(text("SELECT COUNT(*) FROM publications;"))
    ...
    def initialize_database(self) -> bool:
        ...
        if db_info:
            logger.info(f"Banco inicializado - Publicações: {db_info['publication_count']}, "
