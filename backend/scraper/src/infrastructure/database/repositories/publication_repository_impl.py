...
from src.infrastructure.database.models import Publication as PublicationModel, PublicationStatus
...
class PostgreSQLPublicationRepository(PublicationRepository):
    ...
    def save(self, publication: PublicationEntity) -> PublicationEntity:
        ...
        try:
            with db_config.get_session() as session:
                ...
                existing = session.query(PublicationModel).filter(
                    PublicationModel.process_number == publication.process_number
                ).first()
                
                if existing:
                    raise ValueError(f"Publicação com número de processo {publication.process_number} já existe")
                
                # Converte lawyers para formato JSON
                lawyers_json = [lawyer.to_dict() for lawyer in publication.lawyers] if publication.lawyers else None
                ...
                logger.info(f"Publicação salva com sucesso: {publication.process_number}")
                return publication
    ...
    def find_by_process_number(self, process_number: str) -> Optional[PublicationEntity]:
        ...
        try:
            with db_config.get_session() as session:
                db_publication = session.query(PublicationModel).filter(
                    PublicationModel.process_number == process_number
                ).first()
    ...
    def update_status(self, publication_id: str, new_status: str) -> bool:
        ...
        try:
            with db_config.get_session() as session:
                ...
                logger.info(f"Status da publicação {publication_id} atualizado para {new_status}")
                return True
    ...
    def exists_by_process_number(self, process_number: str) -> bool:
        ...
        try:
            with db_config.get_session() as session:
                exists = session.query(PublicationModel).filter(
                    PublicationModel.process_number == process_number
                ).first() is not None
    ...
    def delete_by_id(self, publication_id: str) -> bool:
        ...
        try:
            with db_config.get_session() as session:
                deleted_count = session.query(PublicationModel).filter(
                    PublicationModel.id == publication_id
                ).delete()
    ...
    def _apply_filters(self, query, filters: Dict[str, Any]):
        ...
        if "status" in filters:
            query = query.filter(PublicationModel.status == PublicationStatus(filters["status"]))
        
        if "process_number" in filters:
            query = query.filter(PublicationModel.process_number.ilike(f"%{filters['process_number']}%"))
        
        if "availability_date_from" in filters:
            query = query.filter(PublicationModel.availability_date >= filters["availability_date_from"])
        
        if "availability_date_to" in filters:
            query = query.filter(PublicationModel.availability_date <= filters["availability_date_to"])
        ...
        if "search_term" in filters:
            ...
            query = query.filter(
                or_(
                    ...
                    func.array_to_string(PublicationModel.authors, ',').ilike(search_term)
                )
            )
