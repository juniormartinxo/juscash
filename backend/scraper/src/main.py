...
from src.adapters.secondary.api_repository import APIRepository
from src.adapters.secondary.playwright_scraper import PlaywrightScraper
...
from src.shared.exceptions import ScrapingException
...
class DJEScrapingApplication:
    ...
    def __init__(self):
        ...
        self.scraper_adapter: Optional[PlaywrightScraper] = None
        self.database_adapter: Optional[APIRepository] = None
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.database_adapter = APIRepository(self.settings.API_URL)
            ...
            try:
                ...
                self.cache_adapter = RedisCacheAdapter(
                    ...
                    default_ttl=self.settings.REDIS_TTL,
            ...
            self.scraper_adapter = PlaywrightScraper(
            ...
            self.schedule_use_case = ScheduleScrapingUseCase(
                scheduler=self.scheduler_adapter, scrape_use_case=self.scrape_use_case
            )
    ...
    async def run_immediate_scraping(self) -> dict:
        ...
        stats = await self.scrape_use_case.execute(
            criteria=criteria, skip_duplicates=True
        )
    ...
    async def setup_scheduled_scraping(self) -> str:
        ...
        start_date = datetime.strptime(self.settings.scheduler.start_date, "%Y-%m-%d")
        ...
        job_id = await self.schedule_use_case.setup_daily_execution(
            start_date=start_date, execution_time=execution_time, criteria=criteria
        )
        ...
        logger.info(
            f"📅 Execuções diárias a partir de {start_date.date()} às {execution_time}"
        )
    ...
    async def run_with_scheduler(self) -> None:
        ...
        try:
            ...
            logger.info(
                "⏰ Aplicação rodando em modo agendado. Pressione Ctrl+C para parar."
            )
...
async def main():
    ...
    try:
        ...
        if len(sys.argv) > 1 and "--schedule" in sys.argv:
            ...
            logger.info(
                f"   📋 Publicações encontradas: {stats.get('publications_found', 0)}"
            )
            logger.info(f"   ✅ Publicações novas: {stats.get('publications_new', 0)}")
            logger.info(
                f"   🔄 Publicações duplicadas: {stats.get('publications_duplicated', 0)}"
            )
            logger.info(
                f"   ❌ Publicações com erro: {stats.get('publications_failed', 0)}"
            )
            logger.info(
                f"   ⏱️ Tempo de execução: {stats.get('execution_time_seconds', 0):.2f}s"
            )
