...
class TestScrapePublicationsUseCase:
    ...
    async def test_execute_success(
        self, mock_scraper, mock_database, sample_criteria, sample_publication
    ):
        ...
        use_case = ScrapePublicationsUseCase(
            scraper=mock_scraper, database=mock_database
        )
    ...
    async def test_execute_with_duplicates(
        self, mock_scraper, mock_database, sample_criteria, sample_publication
    ):
    ...
    async def test_execute_handles_scraper_error(
        self, mock_scraper, mock_database, sample_criteria
    ):
        """Testa tratamento de erro no scraper."""
        # Configurar mock para falhar
        mock_scraper.navigate_to_dje.side_effect = DJEScrapingException(
            "Erro de navegação"
        )
...
class TestScheduleScrapingUseCase:
    ...
    async def test_setup_daily_execution(self, mock_database, mock_scraper):
        ...
        schedule_use_case = ScheduleScrapingUseCase(
            scheduler=mock_scheduler, scrape_use_case=scrape_use_case
        )
